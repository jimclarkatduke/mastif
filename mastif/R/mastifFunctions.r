...
mastPlot <- function(output, plotPars = NULL){
 ...
 # pdim <- getPlotDims(xytree,xytrap)
  ...
  mtext('mean (solid), 68% (dashed) of non-zeros',outer=T,side=4, line=-2)
  ...
  if(nspec == 1){
    ...
    par(mfrow=c(2,2), mar=c(2,4,2,.2), bty='n', oma=c(2,2,1,1))
    ...
    mtext('intercept', 1, cex=.9)
    .boxCoeffsLabs( boxPars, specNames, specCol[specNames], cex=.8)
    ...
    if(length(nint) > 0){
      ...
      .boxCoeffs(bc, specNames, xlab = 'Covariates', xaxt='n',
    ...
    if(length(nint) > 0){
      ...
      .boxCoeffs(bc, specNames, xlab = '', xaxt='n',
                 ylab=' ', cols = specCol[specNames], addSpec = '')
    }
  ...
  for(k in 1:nrow(xbar)){
    ...
    qj <- t( apply(kj, 1, quantile, c(.5, .05, .95) ) )
  ...
  #  YR <- T
   ...
   #   if(length(wj) < 3)next
    ...
    if('plotGroups' %in% names(yearEffect)){  # by region
      ...
      file <- paste('yearEffectByRegion.pdf',sep='')
   ...
   #   if(!'plotGroups' %in% names(yearEffect))region <- character(0)
      ...
      for(k in 1:nreg){
        ...
        plot(NULL, xlim = range(yr), ylim = c(-1.2,1.2), xlab = xlab, 
             ylab = '', xaxt='n',yaxt='n')
  ...
  #   wx <- which(xseq < max(ylim))
  #    at <- xseq[ c(wx, (max(wx)+1))]
      ...
      abline(h=0,lty=2,lwd=2,col=.getColor('grey',.8))
      ...
      points(jitter(obs),yObs,pch=16,col=.getColor('forestgreen',.1))
      
      .plotLabel(tplots[j],'topleft')
    }
    mtext('Year',side=1,line=0,outer=T)
    mtext('Residual',side=2,line=0,outer=T)
    mtext('+/- 1 sd',side=4,line=-1,outer=T)
  ...
  if(SPACETIME){
    ...
    for(j in 1:2){
      ...
      mff <- .getPlotLayout(length(mvs$deltaTree))
      
      par(mfrow=mff$mfrow, bty='l',mar=c(4,2,.1,.1), oma=c(4,4,1,1))
   ...
   #       ly[1] <- diff(ly[1:2])/2
    ...
    rownames(tmat) <- names(mvs$scoreTree)
    smat <- tmat
...
firstClean <- function(tdata, sdata, xytree, xytrap,
  ...
  if('repr' %in% names(tdata)){
    ...
    if(rr[2] == 0)stop('every tree declared to be immature')
  }
  ...
  if(max(sdata$active, na.rm=T) > 1 | max(sdata$active, na.rm=T) < 0){
  ...
  tdata$plot  <- as.factor(tdata$plot)
  sdata$plot  <- as.factor(sdata$plot)
  xytree$plot <- as.factor(xytree$plot)
  xytrap$plot <- as.factor(xytrap$plot)
  xytrap$trap <- as.factor(xytrap$trap)
  sdata$trap  <- as.factor(sdata$trap)
  
  tmp <- cleanFactors(sdata)
  
  
  wf <- which( sapply(tdata, is.factor) )
  wf <- wf[!names(wf) %in% c( 'plot', 'treeID', 'species')]
  if(length(wf) > 0){
    for(k in wf){
 ...
 # xytree <- .cleanRows(xytree, 'x')
 # xytree <- .cleanRows(xytree, 'y')
 # xytrap <- .cleanRows(xytrap, 'x')
 # xytrap <- .cleanRows(xytrap, 'y')
...
.mast <- function(inputs, data, formulaFec, formulaRep, predList, yearEffect,
  ...
  treeData$plot <- .fixNames(treeData$plot, all=T)$fixed
  seedData$plot <- .fixNames(seedData$plot, all=T)$fixed
  xytree$plot   <- .fixNames(xytree$plot, all=T)$fixed
  xytrap$plot   <- .fixNames(xytrap$plot, all=T)$fixed
  seedData$trap <- .fixNames(seedData$trap, all=T)$fixed
  xytrap$trap   <- .fixNames(xytrap$trap, all=T)$fixed
  seedNames     <- .fixNames(seedNames, all=T)$fixed
  ...
  if( !ARSETUP ){ 
    ...
    tmp <- firstClean(tdata, sdata, xytree, xytrap, specNames, seedNames)
  ...
  sgibbs <- matrix(0, ng, 2)
  colnames(ugibbs) <- yeGr
  colnames(sgibbs) <- c('sigma','rmspe')
 ...
 #        matrix(0, nseed, length(seedNames)) # seed predictions
  ...
  #    TvarPred[nprob,] <- fmu[obsRows]*zw[obsRows]
  #    TvarEst[nprob,]  <- fg[obsRows]*z[obsRows]
  ...
  mmu <- 1
  if(SAMPR){
    mmu <- matrix( apply(rgibbs[kg,], 2, mean), nrow(R), ncol(R) )
    mse <- matrix( apply(rgibbs[kg,], 2, sd), nrow(R), ncol(R) )
  ...
  fp <- xfec%*%betaFecMu
  if(YR){
    by <- betaYrMu[ yrIndex[,'year'] ] 
    if(RANDYR)by <- by + betaYrRand[ yrIndex[,c('group','year')] ]
    by[is.na(by)] <- 0
    fp <- fp + by
  }
...
meanVarianceScore <- function(output, ktree = 30, maxSite = 100,
  ...
  if(!is.null(seedPredGrid)){
    ...
    for(m in 1:nplot){
      ...
      if( AYR & length(wm) > 0 ){
        ...
        mgrid <- cbind(mgrid, seedPredGrid[wm,areaCols])
  ...
  if(win > 1){
    ...
    for(m in 1:nplot){
      ...
      for(k in 1:nsim){
        ...
        if(LAGMAT){
          ...
          tt <- makeCrossCov( tmat = fm[ff,], win = win, MATRIX=T, COR=F )
          mcov <- tt$lagCov
          tmu  <- tt$lagMean
          tmean <- mean(tmu[,1])
      ...
      #      }
         ...
         #   vars <- mcov[ cbind(zrows, zcols) ]
          ...
          if(k == 1){
            ...
            lagGround <- append(lagGround, list(scor))
          }
        }
        ...
        size[k,] <- c(nrow(Tk),nrow(Ts),nrow(Yk), nrow(Ys), round(carea)) 
    ...
    if(LAGMAT){
      names(lagCanopy) <- names(lagGround) <- rj
      names(scoreT) <- names(scoreS) <- rj
      names(deltaT) <- names(deltaS) <-  rj
    }
    
    rownames(domain) <- rj
    colnames(domain) <- c('trees','sites','treeYr','siteYr','areaM2')
    
    tpr <- paste(rj,'tree-tree',sep='_')
    VtreeXtree <- totalVar[drop=F,tpr,]
    tpr <- paste(rj,'site-site',sep='_')
    VsiteXsite <- totalVar[drop=F,tpr,]
    tpr <- paste(rj,'tree-lag',sep='_')
    VtreeXlag <- totalVar[drop=F,tpr,]
    tpr <- paste(rj,'site-lag',sep='_')
