...
install.packages("~/Documents/GitHub/mast/mastif_1.0.tar.gz", repos = NULL, type = "source")
library("mastif")
library("Rcpp")
library("stringr")
library(RANN)
...
if(user =="chase"){
  ...
  load('../combinedSites/all.treeData.rdata')
  load('../combinedSites/otherTreeData.rdata')
  traitTable <- read.table('../../../traitTable.txt',header=T)
  priorVals  <- read.table("../dataFiles/priorParameters.txt", header = TRUE)
  }
...
output.file.name <- "output.05.16.18"

if(user =="chase"){
  dir.create(paste0('/Volumes/clark/clark.unix/makeMast/comboAnalysis/',output.file.name))
  }else{
...
#21 OK
#22 OK
#23 rr[1,] incorrect dimensions
#24 multiple Unknowns
#25 OK*
#26 OK*
#27 OK
#28 not enough seeds
#29 rr[1,] incorrect dimensions
#30 OK
#31 OK
#32 OK

for(t in 2:length(genFull)){
...
#fixing col names to be compatible with MASTIF
  ...
  if(user =="chase"){
  dir.create(paste0('/Volumes/clark/clark.unix/makeMast/comboAnalysis/',output.file.name))
  setwd(paste0('/Volumes/clark/clark.unix/makeMast/comboAnalysis/',output.file.name))
  }else{
  dir.create(paste0('../comboAnalysis',output.file.name))
  ...
  output <- mastif(formulaFec, formulaRep,  inputs = inputs, ng =200, burnin = 20, yearEffect = yearEffect,randomEffect = randomEffect,predList = predList)
  
    if(user =="chase"){
  save(output, file = paste0('/Volumes/clark/clark.unix/makeMast/comboAnalysis/',output.file.name, "/", genFull[t],"output.rdata"))
  }else{
   save(output, file = paste0('../comboAnalysis',output.file.name,"/",genFull[t],"output.rdata"))
  }
}
```